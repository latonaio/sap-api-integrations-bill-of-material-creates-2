...
import (
	...
	sap_api_output_formatter "sap-api-integrations-bill-of-material-creates/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(header *requests.Header) {
	outputDataHeader, err := c.callBillOfMaterialSrvAPIRequirementHeader("MaterialBOM", header)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callBillOfMaterialSrvAPIRequirementHeader(api string, header *requests.Header) (*sap_api_output_formatter.Header, error) {
	body, err := json.Marshal(header)
	if err != nil {
		return nil, xerrors.Errorf("API request error: %w", err)
	}
	url := strings.Join([]string{c.baseURL, "API_BILL_OF_MATERIAL_SRV", api}, "/")
...
func (c *SAPAPICaller) Item(item *requests.Item) {
	url := fmt.Sprintf("MaterialBOM('%s')/to_Item", item.BillOfMaterial)
	outputDataItem, err := c.callBillOfMaterialSrvAPIRequirementItem(url, item)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callBillOfMaterialSrvAPIRequirementItem(api string, item *requests.Item) (*sap_api_output_formatter.Item, error) {
