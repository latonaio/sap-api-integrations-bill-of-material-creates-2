...
sap-api-integrations-bill-of-material-creates は、外部システム(特にエッジコンピューティング環境)をSAPと統合することを目的に、SAP API で部品表（BOM）データを登録するマイクロサービスです。  
...
以下の sample.json の例は、SAP 部品表（BOM） の ヘッダデータ が登録された結果の JSON の例です。  
...
```
	"cursor": "/Users/latona2/bitbucket/sap-api-integrations-bill-of-material-creates/SAP_API_Caller/caller.go#L50",
